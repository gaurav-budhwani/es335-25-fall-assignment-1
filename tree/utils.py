...
def check_ifreal(y: pd.Series) -> bool:
    ...
    if y_clean.dtype.name == 'category':
        return False
    ...
    # Otherwise, assume it's real/continuous
    return True
...
def entropy(Y: pd.Series) -> float:
    ...
    # Since probs come from value_counts(normalize=True), they should never be 0
...
def information_gain(Y: pd.Series, attr: pd.Series, criterion: str) -> float:
    ...
    """
    # Map the criterion names used by DecisionTree class
    ...
    for _, subset in Y.groupby(attr):
...
def opt_split_discrete(X: pd.DataFrame, y: pd.Series, criterion: str, features: pd.Series):
    best_attr, best_gain = None, -np.inf
    for attr in features:
        gain = information_gain(y, X[attr], criterion)
        if gain > best_gain:
            best_gain = gain
            best_attr = attr
    return best_attr, None  # threshold not needed for discrete
...
def opt_split_real(X: pd.DataFrame, y: pd.Series, criterion: str, features: pd.Series):
    ...
    for attr in features:
        ...
        for thr in thresholds:
            ...
            # Use the same criterion mapping as information_gain function
            if criterion == "information_gain":
                if check_ifreal(y):
                    base_metric = mse(y)
